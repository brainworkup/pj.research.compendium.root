...
#' }
#' @seealso 
#'  \code{\link[tidytable]{mutate}}, \code{\link[tidytable]{case_when}}
...
#'  \code{\link[bwu]{gpluck_make_columns}}, \code{\link[bwu]{gpluck_make_score_ranges}}
#'  \code{\link[tidytable]{mutate}}, \code{\link[tidytable]{case_when}}
#'  \code{\link[glue]{glue}}
#'  \code{\link[dplyr]{relocate}}, \code{\link[dplyr]{filter}}
...
compute_pctile_range <-
  function(.x,
    ...
    if (.score_type == "z_score") {
      ...
      .x <-
        .x |>
        tidytable::mutate(z = (.score - mean(raw_score)) / stats::sd(raw_score)) %>%
        tidytable::mutate(pct1 = round(stats::pnorm(z) * 100, 1)) %>%
        tidytable::mutate(pct2 = tidytable::case_when(
          pct1 < 1 ~ ceiling(pct1),
          pct1 > 99 ~ floor(pct1),
